...
public class MazeGenerator : TileMap
{
    ...
    IDictionary<Vector2, int> nodeDict = new Dictionary<Vector2, int>(); //for nodes, most likely get rid of this tbh and replace with node class
    ...
    private void AddNode(Vector2 nodeLocation)
    {
        if (nodeTilemap.GetCellv(nodeLocation) != node) //makes sure theres no duplicates... in a perfect world i would not need this
        {
            ...
            //nodeDict.Add(nodeLocation,indexPos); put int indexPos = 0; at the top of the script, TBH theres probably a better way to do this...
        }
        else
        {
    ...
    // private Vector2[,] GenerateAdjMatrix()
    // {
    //     Vector2[,] adjMatrix = new Vector2[nodeList.Count, nodeList.Count];
    ...
    //             {
    //                 adjMatrix[i, j] = nodeList[i] - nodeList[j]; //what we get is Vector1 - adjMatrix[i,j] == Vector2
    //             }
    //             else
    //             {
    //                 adjMatrix[i, j] = Vector2.Zero;
    //             }
    //         }
    //     }
